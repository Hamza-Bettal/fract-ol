...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/15 18:19:42 by hbettal           #+#    #+#             */
/*   Updated: 2024/02/08 21:00:13 by hbettal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_fractol(t_cmplx cnst, t_cmplx julia_c, t_mlx mlx)
{
	...
	while (iter++ < MAX_ITER)
	{
		...
		b = z.imag * z.imag;
		if (a + b > 4)
			return (color_changer(iter * 0.7));
		else if (!ft_strcmp(mlx.av[1], "julia"))
			return (color_changer(julia(cnst, julia_c)));
		else
...
void	main_drawer(t_mlx mlx)
{
	...
	while (++y < HEIGHT)
	{
		...
		while (++x < WIDTH)
		{
			cnst.real = (x - WIDTH / 2.0) * 4.0 / WIDTH * mlx.zoom;
			cnst.imag = (y - HEIGHT / 2.0) * 4.0 / HEIGHT * mlx.zoom ;
...
int	main(int ac, char **av)
{
	...
	fail_handling(mlx.image->img_addr);
	main_drawer(mlx);
